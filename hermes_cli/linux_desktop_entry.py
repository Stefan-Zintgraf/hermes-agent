...
def resolve_exec_command(project_root: Optional[Path] = None) -> str:
    ...
    For a source/git installation, prefer its own venv Python.  The desktop
    entry can be regenerated by a previous (broken) entry, in which case
    ``sys.executable`` is an unrelated Python without Hermes dependencies.
    The project venv is the one that was created by the Hermes installer and
    remains stable across menu launches.

    Otherwise prefer the real ``hermes`` executable (argv[0] or PATH). When
    Hermes runs as a module with no launcher installed, use the current
    ...
    if project_root is not None:
        ...
        project_script = project_root / "hermes"
        if (
            venv_python.is_file()
            and os.access(venv_python, os.X_OK)
            and project_script.is_file()
        ):
            # Do not resolve venv_python: it is normally a symlink, and
            # resolving it discards Python's virtual-environment prefix.
            # Running the project script under that interpreter matches the
            # shell installer's known-good launcher exactly.
            argv = [str(venv_python), str(project_script), "desktop"]
